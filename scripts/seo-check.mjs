...
for (const route of checkedRoutes) {
  ...
  if (count(html, /<title>/gi) !== 1)
    failures.push(`${path}: expected exactly one title`);
  if (!title) failures.push(`${path}: missing title`);
  if (!description) failures.push(`${path}: missing meta description`);
  if (canonical !== canonicalForPath(path))
    failures.push(`${path}: incorrect canonical`);
  if (robots !== robotsForRoute(route))
    failures.push(`${path}: incorrect robots directive`);
  if (count(html, /<h1(?:\s|>)/gi) !== 1)
    failures.push(`${path}: expected exactly one H1`);
  if (
    !html.includes('property="og:title"') ||
    !html.includes('name="twitter:title"')
  )
    failures.push(`${path}: missing social metadata`);
  if (
    !html.includes('<div id="root">') ||
    html.includes('<div id="root"></div>')
  )
    failures.push(`${path}: empty rendered app shell`);

  const headingLevels = [...html.matchAll(/<h([1-6])(?:\s|>)/gi)].map((match) =>
    Number(match[1]),
  );
  for (let index = 1; index < headingLevels.length; index += 1) {
    if (headingLevels[index] > headingLevels[index - 1] + 1) {
      failures.push(
        `${path}: heading level skips from H${headingLevels[index - 1]} to H${headingLevels[index]}`,
      );
    }
  }
  ...
  if (seenTitles.has(title))
    failures.push(`${path}: duplicate title with ${seenTitles.get(title)}`);
  else seenTitles.set(title, path);
  if (seenDescriptions.has(description))
    failures.push(
      `${path}: duplicate description with ${seenDescriptions.get(description)}`,
    );
...
for (const route of SEO_ROUTES) {
  const isListed = sitemap.includes(
    `<loc>${canonicalForPath(route.path)}</loc>`,
  );
  if (isListed !== route.indexable)
    failures.push(`${route.path}: sitemap indexability mismatch`);
}

const robotsTxt = await readFile(join(distDir, "robots.txt"), "utf8");
if (
  !robotsTxt.includes("Allow: /") ||
  !robotsTxt.includes(`Sitemap: ${SITE.url}/sitemap.xml`)
) {
