...
export default function MathGrid({ children, className = "" }: MathGridProps) {
  ...
  useEffect(() => {
    ...
    function field(x: number, y: number, currentTime: number) {
      const deltaX = x - pointer.x;
      const deltaY = y - pointer.y;
      const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
      const influence = Math.exp(-distance * 0.008);
      const wave = Math.sin(distance * 0.032 - currentTime * 2.2) * 18 * influence;
      const globalWave = Math.sin(x * 0.014 + currentTime) * Math.cos(y * 0.012 - currentTime * 0.7) * 4;

      return {
        x: (deltaX / (distance + 1)) * wave,
        y: (deltaY / (distance + 1)) * wave + globalWave,
      };
    }
    ...
    function draw() {
      ...
      const spacing = Math.max(34, Math.min(52, width / 16));
      ...
      for (let row = -1; row < rows; row += 1) {
        ...
        for (let column = -1; column < columns; column += 1) {
          ...
          const distortion = field(baseX, baseY, time);
          const x = baseX + distortion.x;
          const y = baseY + distortion.y;

          if (column === -1) ctx.moveTo(x, y);
          else ctx.lineTo(x, y);
        }
      ...
      for (let row = 0; row < rows; row += 1) {
        for (let column = 0; column < columns; column += 1) {
          ...
          const deltaY = y - pointer.y;
          const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
          const opacity = Math.max(0.12, 1 - distance / 330);

          ctx.beginPath();
          ctx.arc(x, y, 1.4 + opacity * 1.8, 0, Math.PI * 2);
          ctx.fillStyle = `rgba(248, 43, 147, ${opacity * 0.9})`;
          ctx.fill();
        }
      }
